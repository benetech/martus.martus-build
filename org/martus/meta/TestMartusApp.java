...
public class TestMartusApp extends TestCaseEnhanced
{
	...
	public void testRetrieveBulletinsBadId() throws Exception
	{
		...
		assertEquals(NetworkInterfaceConstants.INCOMPLETE, retriever.retrieveMyBulletins(badList));
	...
	public void testRetrieveBulletinsEmptyList() throws Exception
	{
		...
		assertEquals("empty", NetworkInterfaceConstants.OK, retriever.retrieveMyBulletins(empty));
	...
	public void testRetrieveBulletins() throws Exception
	{
		...
		Retriever retriever = new Retriever(appWithAccount, null);	
		assertEquals("justB1", NetworkInterfaceConstants.OK, retriever.retrieveMyBulletins(justB1));
		...
		assertEquals("unknownId", NetworkInterfaceConstants.INCOMPLETE, retriever.retrieveMyBulletins(nonExistantUidList));
		...
		assertEquals("retrieve all", NetworkInterfaceConstants.OK, retriever.retrieveMyBulletins(allThree));
	...
	public void testRetrieveBulletinsNoServer() throws Exception
	{
		...
		assertEquals(NetworkInterfaceConstants.NO_SERVER, retriever.retrieveMyBulletins(uploadedIdList));
	...
	{
		TRACE_BEGIN("testGetMyBulletinSummariesNoServer");
		try
		{
			Vector failed = appWithoutServer.getMySummaries();
	...
	{
		TRACE_BEGIN("testGetFieldOfficeBulletinSummariesNoServer");
		try
		{
			Vector failed = appWithoutServer.getFieldOfficeSealedSummaries("");
		...
		try
		{
			Vector failed = appWithoutServer.getFieldOfficeDraftSummaries("");
	...
	{
		TRACE_BEGIN("testGetMyDraftBulletinSummariesErrors");
		assertTrue("must be able to ping", appWithServer.isSSLServerAvailable());
		...
		try
		{
			Vector failed = appWithServer.getMyDraftSummaries();
	...
	{
		TRACE_BEGIN("testGetMyBulletinSummariesErrors");
		assertTrue("must be able to ping", appWithServer.isSSLServerAvailable());
		...
		try
		{
			Vector failed = appWithServer.getMySummaries();
			fail("rejected didn't throw?");
		}
		catch(MartusApp.ServerErrorException ignoreExpectedException)
		{
	...
	{
		TRACE_BEGIN("testGetFieldOfficeBulletinSummariesErrors");
		assertTrue("must be able to ping", appWithServer.isSSLServerAvailable());
		...
		try
		{
			Vector failed = appWithServer.getFieldOfficeSealedSummaries("");
		...
		try
		{
			Vector failed = appWithServer.getFieldOfficeDraftSummaries("");
	...
	{
		TRACE_BEGIN("testGetMySummaries");
	...
	{
		TRACE_BEGIN("testGetMyDraftSummaries");
	...
	{
		TRACE_BEGIN("testGetFieldOfficeSummaries");
		...
		Vector returnedSealedResults = hqApp.getFieldOfficeSealedSummaries(b1.getAccount());
		...
		Vector returnedDraftResults = hqApp.getFieldOfficeDraftSummaries(b3.getAccount());
	...
	public void testDownloadFieldOfficeBulletins() throws Exception
	{
		...
		assertEquals("retrieve field office bulletins failed?", NetworkInterfaceConstants.OK, retriever.retrieveFieldOfficeBulletins(uidList));

		uidList.clear();
		uidList.add(b3.getUniversalId());
		assertEquals("retrieve non-field office bulletins worked?", NetworkInterfaceConstants.INCOMPLETE, retriever.retrieveFieldOfficeBulletins(uidList));
