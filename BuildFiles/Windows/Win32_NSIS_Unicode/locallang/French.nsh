...
!insertmacro LANG_STRING UninstallMartusManuallyAndRemoveLinks_Text "Vous ne pouvez pas mettre à jour à partir des versions de Martus âgés de plus de 4.3 vers la version actuelle. $ \ R $ \ n $ \ r $ \ nVous devez désinstaller votre ancienne version de Martus (ce soit sous Démarrer > Programmes > Martus > Désinstaller Martus, ou en utilisant Ajout/Suppression de Programmes du Panneau de Configuration). $ \ r $ \ n $ \ r $ \ nSi vous utilisez Windows Vista ou Windows 7 et a eu un raccourci Martus dans votre menu Démarrer, vous pouvez aussi avoir besoin de retirer le groupe Martus en allant Démarrer > Programmes, clic droit sur le groupe Martus, et la cueillette 'Supprimer'. $ \ r $ \ nSi non, ces liens resteront lorsque vous désinstallez la version actuelle de Martus, qui pourrait être un problème de sécurité."
