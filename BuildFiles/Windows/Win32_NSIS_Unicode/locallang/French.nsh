...
!insertmacro LANG_STRING UninstallMartusManuallyAndRemoveLinks_Text "Vous ne pouvez pas mettre à jour vers la version actuelle à partir d'une version de Martus plus ancienne que la version 4.3.$\r$\n $\r$\nVous devez désinstaller votre ancienne version de Martus (que ce soit sous Démarrer > Programmes > Martus > Désinstaller Martus, ou en utilisant Ajouter/Suppression de programmes du Panneau de configuration).$\r$\n $\r$\nSi vous utilisez Windows Vista ou Windows 7 et avez raccourci Martus dans votre menu Démarrer, vous pouvez aussi avoir besoin de retirer le groupe Martus en allant Démarrer > Programmes, clic droit sur le groupe Martus, et choisissant 'Supprimer'.$\r$\nSinon ces liens resteront lorsque vous désinstallez la version actuelle de Martus, ce qui pourrait causer un problème de sécurité."
