...
public:
	...
	void setDestinationFilename( const std::string dname ); // set destination filename
	...
	std::string getDestinationFilename() const { return destinationFilename; } // return filename where file will be split/merged to
...
private:
	...
	std::string destinationFilename; // holds destination filename of file to be split/merged
    ...
    int readIntoStream( const std::string &str, FILE *outfile, const int fnum ) const; // used for merging
    int writeIntoStream( const std::string &str, FILE *infile ) const; // used for splitting
