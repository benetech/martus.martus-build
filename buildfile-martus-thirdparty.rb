...
#	install artifact(ICU4J_SOURCE_SPEC).from(source_file(name, 'common/PersianCalendar', 'icu4j_3_2_license.html'))
	...
	install artifact(ORCHID_SPEC).from(jar_file(name, 'common/orchid', "orchid-#{$ORCHID_VERSION}.b4a87ff.jar"))
	install artifact(ORCHID_SOURCE_SPEC).from(source_file(name, 'common/orchid', "orchid-#{$ORCHID_VERSION}.b4a87ff-src.zip"))
