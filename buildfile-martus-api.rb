...
define name, :layout=>create_layout_with_source_as_source('.') do
	...
	package(:zip, :file=>jarname).tap do | p |
		...
		p.merge(project('martus-utils').package).include('org/martus/utils/*.class')
		p.merge(project('martus-utils').package).include('org/martus/utils/inputstreamwithseek/*.class')
		p.merge(project('martus-utils').package).include('org/martus/utils/language/*.class')
		p.merge(project('martus-utils').package).include('org/martus/utils/xml/*.class')
